...
//WIRES
  logic unit_rst_n;
  logic w_en_lfsr, w_en_apuf_path, w_en_sipo;
  logic [APUF_DATA_BITS_OUT - 1:0] data_to_send;

//REGS
  logic tx_dv_ff,   tx_dv_next;
  logic [APUF_DATA_BITS_OUT - 1:0] tx_data_ff, tx_data_next;
  
  logic [APUF_DATA_BITS_IN - 1 + 8 :0] data_counter_ff, data_counter_next;
    ...
    unique case (apuf_state_ff) 
      ...
      APUF_EXP: begin
        if (cycles_counter_ff == 'b0) begin
          apuf_state_next = APUF_BIT;
        end  
      end
      
      APUF_BIT: begin
        if (bit_counter_ff != 'b1) begin
          apuf_state_next = APUF_EXP;
        end
        else if ((bit_counter_ff == 'b1) && (tx_active_i == 'b0)) begin
...
//PROC_LOGIC_CTRL
  always_comb begin : proc_logic_ctrl 
    ...
    bit_counter_next  = bit_counter_ff; 
    ...
    unique case (apuf_state_ff) 
      ...
      APUF_CYCLE: begin
        ...
        if (data_counter_ff != 'b0) begin
          ...
          bit_counter_next    = APUF_DATA_BITS_OUT;
      ...
      APUF_BIT: begin
        if (bit_counter_ff != 'b0) begin
          cycles_counter_next = APUF_CYCLES_COUNT;

          bit_counter_next = unsigned'(bit_counter_ff - 'b1);
          w_en_lfsr      = 'b1;
          w_en_sipo      = 'b1;
        end
      end
...
//UNIT INSTS
  ...
  sipo #(
    .SIPO_REG_W(APUF_DATA_BITS_OUT)
  ) sipo_inst_0 ( 
    .clk(clk),
    .rst_n(unit_rst_n),
    .w_en_i(w_en_sipo),
    .serial_i(response_inter),
    .parallel_o(data_to_send)
  );
